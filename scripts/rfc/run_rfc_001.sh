#!/bin/bash
# ============================================================
# SNOP_RFC_001: 자재마스터 동기화 (SAP → S&OP)
# 대상테이블: MOD_SALES_BASE_MATERIAL_MASTER
# 처리방식: new_update_type 1=Insert, 2=Update by item_code
# 등록자/수정자: IF, 등록일시/수정일시 자동기록
# ============================================================
# 사용법: 인터페이스 마스터관리 > 실행명령어 필드에 아래 경로 등록
#   /home/user/webapp/scripts/rfc/run_rfc_001.sh
# ============================================================
...
SOP_RFC_ENDPOINT="${SOP_BASE_URL}/api/rfc/SNOP_RFC_001"
SAP_RFC_URL="${SAP_RFC_URL:-}"
RFC_PARAM="${RFC_PARAM:-A}"
LOG_PREFIX="[RFC-001]"

echo "${LOG_PREFIX} 자재마스터 동기화 시작 - $(date '+%Y-%m-%d %H:%M:%S')"
...
if [ -n "${SAP_RFC_URL}" ]; then
    ...
    SAP_RESPONSE=$(curl -s -X POST "${SAP_RFC_URL}" \
        ...
        -d "{\"rfc_id\":\"SNOP_RFC_001\", \"param\":\"${RFC_PARAM}\"}" \
        --connect-timeout 30 \
        --max-time 300)
...
if [ "${STATUS}" = "SUCCESS" ] || [ "${STATUS}" = "PARTIAL_SUCCESS" ]; then
    echo "${LOG_PREFIX} 자재마스터 동기화 완료 - $(date '+%Y-%m-%d %H:%M:%S')"
    exit 0
else
    echo "${LOG_PREFIX} 자재마스터 동기화 실패 - $(date '+%Y-%m-%d %H:%M:%S')"
    exit 1
fi
