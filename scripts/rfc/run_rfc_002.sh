#!/bin/bash
# ============================================================
# SNOP_RFC_002: 일자별재고 동기화 (SAP → S&OP)
# 대상테이블: MOD_PLANT_STORAGE_LOCATION
# 처리방식: plan_month_day 앞 6자리 동일한 기존레코드 삭제 후 전체 신규 insert
# 등록자/수정자: IF, 등록일시/수정일시 자동기록
# ============================================================
# 사용법: 인터페이스 마스터관리 > 실행명령어 필드에 아래 경로 등록
#   /home/user/webapp/scripts/rfc/run_rfc_002.sh
# ============================================================
...
if [ -n "${SAP_RFC_URL}" ]; then
    ...
    # Step 2: S&OP API로 전송 (기존 레코드 삭제 + 신규 insert)
...
if [ "${STATUS}" = "SUCCESS" ] || [ "${STATUS}" = "PARTIAL_SUCCESS" ]; then
    echo "${LOG_PREFIX} 일자별재고 동기화 완료 - $(date '+%Y-%m-%d %H:%M:%S')"
    exit 0
else
    echo "${LOG_PREFIX} 일자별재고 동기화 실패 - $(date '+%Y-%m-%d %H:%M:%S')"
    exit 1
fi
