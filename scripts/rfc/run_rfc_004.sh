#!/bin/bash
# ============================================================
# SNOP_RFC_004: 판매실적 동기화 (SAP → S&OP)
# 대상테이블: MOD_SALES_SNOP_RECORD
# 처리방식: plan_month+item_code 매칭건 update
# 등록자/수정자: IF, 등록일시/수정일시 자동기록
# ============================================================
# 사용법: 인터페이스 마스터관리 > 실행명령어 필드에 아래 경로 등록
#   /home/user/webapp/scripts/rfc/run_rfc_004.sh
# ============================================================
...
if [ "${STATUS}" = "SUCCESS" ] || [ "${STATUS}" = "PARTIAL_SUCCESS" ]; then
    echo "${LOG_PREFIX} 판매실적 동기화 완료 - $(date '+%Y-%m-%d %H:%M:%S')"
    exit 0
else
    echo "${LOG_PREFIX} 판매실적 동기화 실패 - $(date '+%Y-%m-%d %H:%M:%S')"
    exit 1
fi
